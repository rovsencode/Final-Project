...
function ActressCreate() {
  ...
  const handleSubmit = async (values) => {
    const { data } = await axios.post("https://localhost:7152/api/Actress/Create", values);
    if (data.errors === null) {
      setSuccessSB(true);
      setTimeout(() => {
        navigate("/genre");
